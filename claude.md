...
- In the case where an outcome we hadn't prepared for occurs, we fallback to a backup speculation option. The options are "jit", in which we do regular speculation for the whole batch "just-in-time", and "fast", in which the batch elements that had a "cache miss" fall back to their own sequence's first cached branch (the k=0 top-fork speculation from the previous round — a cheap fallback that still earns accepted tokens after a near-miss), or to a speculation of all zeros when the sequence has no cache entries (e.g., its first round). We also implement a "force-jit" option, which always does JIT speculation, regardless of cache hit vs miss. This is to allow direct comparison with regular (synchronous) speculative decoding for debugging purposes.
